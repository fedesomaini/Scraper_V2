...
namespace iReg
{
    ...
    internal class CsvStream
    {
        ...
        public CsvStream(char SeparatorChar, bool IgnoreTrailingSeparatorChar)
        {
            m_SeparatorChar = SeparatorChar;
            m_IgnoreTrailingSeparatorChar = IgnoreTrailingSeparatorChar;
            m_SpecialChars = ("\"\x0A\x0D" + m_SeparatorChar.ToString()).ToCharArray();
            ...
            foreach (string item in row)
            {
                ...
                if (item != null)
                {
                    ...
                    if ((quoteAllFields ||
                        ...
                        (item.Trim() == "")))
                    {
                        m_outStream.Write("\"" + System.Net.WebUtility.HtmlDecode(item).Replace("\"", "\"\"") + "\"");
                    }
                    else
                    {
                        m_outStream.Write(System.Net.WebUtility.HtmlDecode(item));
                    }
                }
        ...
        public string OutputRow(List<string> row, bool quoteAllFields)
        {
            bool firstItem = true;
            StringBuilder sb = new StringBuilder();
            foreach (string item in row)
            {
                if (!firstItem) { sb.Append(m_SeparatorChar); }
                ...
                if (item != null)
                {
                    ...
                    if ((quoteAllFields ||
                        ...
                        (item.Trim() == "")))
                    {
                        sb.Append("\"" + System.Net.WebUtility.HtmlDecode(item).Replace("\"", "\"\"") + "\"");
                    }
                    else
                    {
                        sb.Append(System.Net.WebUtility.HtmlDecode(item));
                    }
                }
